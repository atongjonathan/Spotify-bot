...
class SGBot():
    ...
    def reply_to_query(self,message,reply_text,search_function):
        self.BOT.reply_to(message,reply_text,reply_markup=self.keyboard.force_markup)
    ...
    def search_trending(self, message, no_of_songs):
        reply = self.BOT.send_message(message.chat.id, "Awesome getting trending somgs in a few")
        hot_100 = billboard.ChartData("hot-100")[:no_of_songs]
        track_details = [self.spotify.song(artist=item.artist,title=item.title)[0] for item in hot_100]
        result_string = [f'{idx+1}. `{item["name"]}` - {item["artists"]}' for idx, item in enumerate(track_details)]
        result_string = '\n'.join(result_string)
        artists_keyboard = self.keyboard.keyboard_for_results(results=track_details)
    ...
    def send_top_songs(self, call):
        ...
        for track in top_tracks:
            track_details = self.spotify.song(artist_details["name"], track, None)
            try:
    ...
    def search_song(self, message, query=None):       
        ...
        result_string = '\n'.join(result_string)
        artists_keyboard = self.keyboard.keyboard_for_results(results=possible_tracks)
        self.BOT.send_message(
            message.chat.id,
    ...
    def search_artist(self,message, artist=None) -> None:
        ...
        artists_keyboard = self.keyboard.keyboard_for_results(results=artist_results)
        self.BOT.send_message(
            message.chat.id,
    ...
    def send_audios_or_previews(self, track_details, caption, chat_id, send_photo):
        ...
        update = self.BOT.send_message(chat_id, f"...⚡Downloading track no {track_details['track_no']} - `{title}`⚡ ...")
        if self.isPreview:
            if track_details['preview_url'] is None:
                ...
                self.BOT.send_audio(chat_id,
                            ...
                            performer=track_details['artists'].replace(" ", ""),
    ...
    def get_album_songs(self, uri, chat_id):
        ...
        if isinstance(album_details, str):
            ...
            for track in album_tracks:
                ...
                self.send_audios_or_previews(track_details, caption, chat_id, False)
            self.BOT.send_message(
                chat_id,
    ...
    def send_checker(self, list_of_type: list, chat_id: str, current_page: int):
        ...
        """
        try:
            reply_markup = self.keyboard.make_for_type(list_of_type, current_page)
        except BaseException:
    ...
    def process_callback_query(self, call):
        ...
        if data.startswith('album') or data.startswith('single') or data.startswith(
            ...
            self.BOT.answer_callback_query(call.id)  # Handle for closing all Inline markups
    ...
    def handle_lyrics_callback(self, call):
        ...
        if lyrics == None or lyrics == "":
            self.BOT.answer_callback_query(call.id, text=f"'{title}' lyrics not found!",show_alert=True)
        else:
            self.BOT.answer_callback_query(call.id)
