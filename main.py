...
def search_artist(message):
    if spotify.get_details_artist(message.text) == None:
        bot.send_message(message.chat.id,
        "Artist not found!⚠. Make sure to include all artist name properties including supersripts.\nTry again? /artist")
        return
    artist_uri, followers, images, name, genres = spotify.get_details_artist(message.text)
    try:
        image = images[0]
    except IndexError:
    ...
    pin = bot.send_photo(message.chat.id, photo=image, caption=caption, reply_markup=keyboard.view_handler(name,artist_uri,list_of_albums,list_of_singles))
...
def send_song_data(message):
    ...
    try:
        ...
        artist = ""
    artist, preview_url, release_date, album, track_no, total_tracks,image,id = spotify.get_track_details(artist,song)
...
def send_audios_or_previews(preview_url, image, caption, name, id, artist, chat_id,send_photo):
    ...
    if send_photo:
        time.sleep(1.5)
        bot.send_photo(chat_id, photo=image, caption=caption, reply_markup=keyboard.lyrics_handler(artist,name))
    update = bot.send_message(chat_id, "... Downloading song just a sec ...")
    artist, preview_url, release_date, album, track_no,total_tracks,image,track_id = spotify.get_track_details(artist,name)
    ...
    if data is not None:
        ...
        audio.set_metadata(artist,release_date,album,name,track_no,image,files[0])
        path = files[0]
        with open(path, "rb") as file:
            bot.send_chat_action(chat_id, "upload_audio")
            bot.send_audio(chat_id, audio=file, title=f'{name}', performer=artist, reply_markup=keyboard.start_markup, caption="@JonaAtong")
        os.remove(path)
        ...
        bot.send_chat_action(chat_id, "upload_audio")
        bot.send_audio(chat_id, audio=audio_io, title=f'{name}', performer=artist, reply_markup=keyboard.start_markup)
...
def get_album_songs(small_uri,chat_id, list_of_albums):
    for idx, album in enumerate(list_of_albums):
        if small_uri in album['uri'].split(":")[2]:
            ...
            chosen_album = album
    release_date, total_tracks, photo, track_list = spotify.get_album_details(small_uri)
    ...
    for track in album_tracks:
        name = track['name']
        artist = track["artist"]
        artist, preview_url, release_date, album, track_no, total_tracks,image,id = spotify.get_track_details(artist,name)
        caption = f"👤Artist: {artist}\n🔢Track : {track_no} of {total_tracks}\n🎵Song : {name}\n"
        send_audios_or_previews(preview_url, photo, caption, name, id, artist, chat_id,False)

    bot.send_message(chat_id, f'Those are all the {total_tracks} track(s) by {artist} in "{album_name}" 💪!',reply_markup=keyboard.start_markup)
...
def get_top_tracks(chat_id, uri):
    ...
    bot.send_message(chat_id, f"Those are {artist}'s top 🔝 {spotify.no_of_songs} tracks 💪!", reply_markup=keyboard.start_markup)
...
def send_lyrics(message):
    ...
    try:
        ...
        artist = ""
    artist, preview_url, release_date, album, track_no,total_tracks,image,id = keyboard.get_track_details(artist,title)
...
def artist(message):
    bot.send_message(message.chat.id, "Send me the name of the artist", reply_markup=keyboard.force_markup)
    bot.register_next_step_handler_by_chat_id(message.chat.id, lambda msg: search_artist(msg))
...
def get_song(message):
    bot.send_message(message.chat.id, "Awesome, send the name of the song with the artist separated by a comma", reply_markup=keyboard.force_markup)
...
def handle_query(call):
    ...
    if call.data.startswith('single_'):
        ...
        artist, preview_url, release_date, album, track_no,total_tracks,image,id = spotify.get_track_details(artist,title)
        lyrics = extract_lyrics.get_lyrics(f"{title} {artist}")["lyrics"]
        caption = f"👤Artist: {artist}\n🎵Song : {title.title()}\n━━━━━━━━━━━━\n📀Album : {album}\n🔢Track : {track_no} of {total_tracks}\n⭐️ Released: {release_date}\n\n🎶Lyrics:\n{lyrics}"
        try:
