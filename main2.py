...
def search_artist(message) -> None:
    ...
    top_tracks = artist_details["top_songs"]
    singles = artist_details["singles"]
    ...
    if artist_details is None:
        ...
        return
    caption = f'👤Artist: {artist_details["name"]}\n🧑Followers: {artist_details["followers"]:,} \n🎭Genre(s): {", ".join(artist_details["genres"])} \n'
    pin = bot.send_photo(message.chat.id, photo=artist_details["images"], caption=caption, reply_markup=keyboard.view_handler(
        artist_details["name"], singles, albums))
...
def handle_query(call):
    if call.data.startswith('albums'):
